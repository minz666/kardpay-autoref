...
def get_log_prefix(account_num=None):
    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    if account_num is not None:
        return f"[{Fore.LIGHTBLUE_EX}{timestamp}{Style.RESET_ALL} #{account_num}]"
...
def save_account(email, password, account_num):
    with open('accounts.txt', 'a') as f:
        f.write(f"{email}|{password}\n")
    print(f"{get_log_prefix(account_num)} {Fore.LIGHTMAGENTA_EX}Complete! Account saved to accounts.txt{Style.RESET_ALL}")

def send_otp(email, password, proxy_dict, account_num):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"{get_log_prefix(account_num)} {Fore.LIGHTGREEN_EX}Success Register{Style.RESET_ALL}")
            return result
        else:
            print(f"{get_log_prefix(account_num)} {Fore.LIGHTRED_EX}Register failed: {result}{Style.RESET_ALL}")
            return None
    except requests.RequestException as e:
        print(f"{get_log_prefix(account_num)} {Fore.LIGHTRED_EX}Request error: {str(e)}{Style.RESET_ALL}")
        return None

def login(id_token, proxy_dict, account_num):
    ...
    try:
        ...
        print(f"{get_log_prefix(account_num)} {Fore.LIGHTRED_EX}Login error: {str(e)}{Style.RESET_ALL}")
        return None

def set_reff(referral_code, token, proxy_dict, account_num):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"{get_log_prefix(account_num)} {Fore.LIGHTRED_EX}Failed to referral: {response.text}{Style.RESET_ALL}")
            return False
    except requests.exceptions.RequestException as e:
        print(f"{get_log_prefix(account_num)} {Fore.LIGHTRED_EX}Referral error: {str(e)}{Style.RESET_ALL}")
        return False
...
def process_single_registration(proxies, referral_code, account_num):
    ...
    while True:
        ...
        if not email:
            print(f"{get_log_prefix(account_num)} {Fore.LIGHTRED_EX}Failed to generate email. Retrying...{Style.RESET_ALL}")
            time.sleep(2)
            continue

        password = generate_password()
        print(f"{get_log_prefix(account_num)} {Fore.LIGHTYELLOW_EX}Email: {email}{Style.RESET_ALL}")
        print(f"{get_log_prefix(account_num)} {Fore.LIGHTYELLOW_EX}Password: {password}{Style.RESET_ALL}")

        otp_result = send_otp(email, password, proxy_dict, account_num)
        if not otp_result:
            time.sleep(2)
            continue
        ...
        if not id_token:
            print(f"{get_log_prefix(account_num)} {Fore.LIGHTRED_EX}No ID token received{Style.RESET_ALL}")
            time.sleep(2)
            continue

        login_result = login(id_token, proxy_dict, account_num)
        if not login_result:
            time.sleep(2)
            continue

        if referral_code.strip():
            if not set_reff(referral_code, id_token, proxy_dict, account_num):
                time.sleep(2)
                continue

        save_account(email, password, account_num)
        return True
...
def main():
    ...
    while True:
        attempt_count += 1
        print(f"{get_log_prefix(current_account)} {Fore.LIGHTCYAN_EX}{'='*50}{Style.RESET_ALL}")

        if process_single_registration(proxies, referral_code, current_account):
            print(f"{get_log_prefix(current_account)} {Fore.LIGHTBLUE_EX}Referral Complete for Account #{current_account}{Style.RESET_ALL}")
            current_account += 1
